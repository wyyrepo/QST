...
QueryFrame *PostgreSqlBuilder::makeKeywordFrame(QueryFrame *subqueryFrame,
...
void PostgreSqlBuilder::makeClauseItemFrame(QueryFrame *parentFrame,
...
QueryBatchList PostgreSqlBuilder::validBatchList(const Qst::QueryClause &queryClause,
...
QString PostgreSqlBuilder::_composeCondition(const QueryField &field) const
{
	if (field.isValuesValid())
	{
		...
		QString strCond = _valueFormat.toString(field.value(0), ToStringTypes_Mask,
												field.value(0).percentPlaceholders(), true);
		return conditionTemplate.string(field.name(), strCond);
	}
	else
...
QueryFrame PostgreSqlBuilder::_conditionClauseFrame(const QueryWhere &cond) const
{
	QueryFrame resFrame;
											 ...
											 const StringTemplate &strTempl) const
{
	...
	QString strVal1 = _valueFormat.toString(val1, ToStringTypes_Mask,
											val1.percentPlaceholders(), true);
	QString strVal2 = _valueFormat.toString(val2, ToStringTypes_Mask,
											val2.percentPlaceholders(), true);
											...
											const StringTemplate &strTempl) const
{
	...
	foreach (QueryValue val, field.valueList())
		list << _valueFormat.toString(val, ToStringTypes_Mask,
									  NoPercentPlaceholders, true);
